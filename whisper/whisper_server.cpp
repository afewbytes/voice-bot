...
// and support for the new StreamAudioResponse
...
grpc::Status WhisperServiceImpl::StreamAudio(
    ...
    auto process_buffer = [&](bool is_final) -> bool {
        ...
        if (!out_text.empty()) {
            // Create the new StreamAudioResponse
            voice::StreamAudioResponse response;
            response.set_text(out_text + "\n");
            response.set_source(voice::StreamAudioResponse_Source_WHISPER);
            response.set_done(is_final); // Set done flag based on whether this is the final segment
            
            LOG_INFO("Session " << session_id << ": Sending transcription: \""
                    << out_text << "\", final: " << (is_final ? "true" : "false"));
