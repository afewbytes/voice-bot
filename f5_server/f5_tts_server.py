...
# ─── F5-TTS import chain ───────────────────────────────────────────────────
try:
    from f5_tts import TTS as _TTS                       # ≤0.6.x
except ImportError:
    try:
        from f5_tts.infer.api import TTS as _TTS         # dev layout
    except ImportError:
        from f5_tts.api import F5TTS as _TTS             # ≥1.0.0
TTS = _TTS
...
def _load_engine(a) -> TTS:
    if "ckpt_file" in inspect.signature(TTS).parameters:
        ckpt = (next((os.path.join(a.checkpoint, f)
                     for f in os.listdir(a.checkpoint) if f.endswith(".safetensors")),
                     a.checkpoint) if os.path.isdir(a.checkpoint) else a.checkpoint)
...
async def serve(a):
    ...
    # run warm-up in the background so health-check passes immediately
